...
int main(void)
{
    ...
    while(1)
    {
        if (BSP::tim2::getTimeout())
        {
            ...
            const int freq_lut_idx_ui10 = BSP::adc1::getInput_ui12(BSP::adc1::LFO_FREQ) >> 2;
            lfo.setInput(LFO::FREQ_mHz, Lookup_Tables::EXPO_MAPPING_LUT_mHz[freq_lut_idx_ui10]);

            // update the LFO wave scanner
            const int wave_scan_ui16 = BSP::adc1::getInput_ui12(BSP::adc1::LFO_SHAPE) << 4;
            ...
            scaled_lfo *= BSP::adc1::getInput_ui12(BSP::adc1::LFO_LEVEL);
